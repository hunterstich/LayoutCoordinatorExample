...
public class SlidingTabLayout extends HorizontalScrollView {
    ...
    private float mTotalTabsWidth = 0;
    ...
    public void changeTabTitleTextColor(int color) {
        for (int i = 0; i < mAllTabs.size(); i++) {
            TextView textView = (TextView) mAllTabs.get(i).findViewById(mTabViewTextViewId);
            textView.setTextColor(color);
        }
    }

    public void setOnPageChangeListener(ViewPager.OnPageChangeListener listener) {
        mViewPageChangeListener = listener;
    }
    ...
    private void populateTabStrip() {
        ...
        for (int i = 0; i < adapter.getCount(); i++) {
            ...
            String desc = mContentsDescriptions.get(i, null);
            if (desc != null) {
    ...
    public void setContentDescription(int i, String desc) {
        mContentsDescriptions.put(i, desc);
    }
    ...
    private class InternalViewPageListener implements ViewPager.OnPageChangeListener {
        private int mScrollState;

        @Override
        public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
            int tabStripChidCount = mTabStrip.getChildCount();
            if ((tabStripChidCount == 0) || (position < 0) || (position >= tabStripChidCount)) {
                return;
            }
        ...
        public void onPageScrollStateChanged(int state) {
            ...
            if (mViewPageChangeListener != null) {
                mViewPageChangeListener.onPageScrollStateChanged(state);
            }
        }
